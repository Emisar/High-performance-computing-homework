function task1(task) {
    ...
    const ANSWER_TRUE = "Может быть матрицей смежности!";
    const ANSWER_FALSE = "Не может быть матрицей смежности!";
    ...
    function checkMatrix(matrix) {
        ...
        for (let i = 0; i < height; i++) {
            for (let j = 0; j < width; j++) {
                ...
                // Проверяем матрицу на наличее других значений помимо 0 и 1 (матрица смежности)
                if (elem1 < 0 && elem1 > 1) return false;
                // Проверяем главную диагональ матрицы (наличее петель)
                if (i == j && elem1 == 1) return false; 
                // Проверяем зеркальные значения относительно главной диагонали (наличее только одного ребра)
                if (i != j && elem1 == 1 && elem1 == elem2) return false;
            }
        }
